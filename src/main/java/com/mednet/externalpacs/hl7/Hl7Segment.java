...
public class Hl7Segment {
    ...
    /**
     * XCN id (component 1), or NDL id (first subcomponent of component 1).
     */
    public String personId(int fieldNumber) {
        String first = component(fieldNumber, 1);
        if (isBlank(first)) {
            return "";
        }
        if (first.contains("&")) {
            return part(first.split("&", -1), 0);
        }
        return first;
    }

    /**
     * XCN: family^given^middle. NDL: id&family&given.
     * Fenix often sends the full name in XCN-2 (e.g. tuliroy^Dr. Tuli Roy).
     */
    public String personDisplayName(int fieldNumber) {
        String first = component(fieldNumber, 1);
        String family;
        String given;
        String middle;
        if (!isBlank(first) && first.contains("&")) {
            String[] parts = first.split("&", -1);
            family = part(parts, 1);
            given = part(parts, 2);
            middle = part(parts, 3);
        } else {
            family = component(fieldNumber, 2);
            given = component(fieldNumber, 3);
            middle = component(fieldNumber, 4);
        }
        return firstNonBlank(joinName(given, middle, family), family, given);
    }

    /**
     * First-level {@code &} split of the whole field (NDL-style), 1-based.
     */
    public String subcomponent(int fieldNumber, int oneBasedIndex) {
        String value = field(fieldNumber);
        if (isBlank(value)) {
            return "";
        }
        return part(value.split("&", -1), oneBasedIndex - 1);
    }
    ...
    private static String joinName(String given, String middle, String family) {
        StringBuilder sb = new StringBuilder();
        appendName(sb, given);
        appendName(sb, middle);
        appendName(sb, family);
        return sb.toString().trim();
    }

    private static void appendName(StringBuilder sb, String part) {
        if (isBlank(part)) {
            return;
        }
        if (sb.length() > 0) {
    ...
    private static String part(String[] parts, int index) {
        ...
        return parts[index] == null ? "" : parts[index];
    }
