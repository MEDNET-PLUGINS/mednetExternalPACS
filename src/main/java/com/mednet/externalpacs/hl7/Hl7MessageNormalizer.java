...
public final class Hl7MessageNormalizer {
    ...
    /**
     * Splits a dump that contains more than one HL7 message (multiple {@code MSH} / MLLP frames).
     */
    public static List<String> splitMessages(String raw) {
        if (raw == null || raw.isBlank()) {
            return List.of();
        }
        String payload = raw.replace('\u000b', '\n').replace('\u001c', '\n');
        String[] parts = payload.split("(?=MSH\\|)", -1);
